...
const Task = ({ taskList }) => {
  ...
  const taskForToday = taskList.find(
    (item) => formatDate(item.start_date, item.duration) === today
  );
        ...
        <ol className="mt-2 divide-y divide-gray-200 text-sm leading-6 text-gray-500">
          ...
          {Object.entries(dates).map(([formattedDate, tasks]) => (
            <li key={formattedDate} className="py-4 sm:flex">
              <time
                dateTime={tasks[0].start_date}
                className="w-28 flex-none text-left"
              >
                {formattedDate}
              </time>
              <ul>
                {tasks.map((task) => (
                  <li key={task.id}>
                    <p className="mt-2 flex-auto font-semibold text-gray-900 sm:mt-0 text-left">
                      {task.task_name}
                    </p>
                  </li>
                ))}
